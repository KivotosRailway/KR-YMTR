...
@Pseudo
@Mixin(value = {Catenary.class}, remap = false)
public class MSDCatenaryMixin {

	@ModifyArg(method = {"render"}, at = @At(value = "INVOKE",
			target = "Ltop/mcmtr/data/Catenary;renderSegment(Ltop/mcmtr/data/Catenary$RenderCatenary;)V",
			remap = false), index = 0, remap = false)
	private Catenary.RenderCatenary wrapRenderCallback(Catenary.RenderCatenary callback) {
		final Vec3 cameraPos = Minecraft.getInstance().gameRenderer.getMainCamera().getPosition();
		return (x1, y1, z1, x2, y2, z2, count, i, base, sinX, sinZ, increment) -> {
			RenderTrains.MSD_CAMERA_RELATIVE.set(true);
			try {
				callback.renderCatenary(
